...
function buildAzizi(){
  ...
  /* --- Schwert in der rechten Hand -------------------------------
     Wie bei Abdus Axt: der Arm hängt in Grundstellung herab (poseR ≈ 0),
     die Klinge steht über das Waffengelenk aufrecht nach oben. So bricht
     sie die Silhouette nach oben und ist aus der Vogelperspektive klar zu
     sehen — eine nach vorn gelegte Klinge verschwindet dagegen vor dem
     Körper. Der Angriffsschwung dreht den Arm nach vorn und führt die
     Klinge als Hieb herunter. Leicht nach außen-hinten gekippt, damit sie
     den Kopf nicht schneidet, und bewusst lang, damit sie bei der kleinen
     Figur überhaupt auffällt. */
  const sword = joint(0.02*s, -0.25*s, 0.02*s);
  sword.rotation.set(-0.16, 0, -0.42);
  armR.add(sword);
  sword.add(part(box(0.038*s, 0.58*s, 0.017*s), metal(PAL.steelLite, 0.22), 0, 0.33*s, 0));  // Klinge
  sword.add(part(spike(0.032*s, 0.09*s, 4), metal(PAL.steelLite, 0.22), 0, 0.66*s, 0));       // Spitze
  sword.add(part(box(0.145*s, 0.032*s, 0.036*s), metal(AZI.gold, 0.3), 0, 0.035*s, 0));        // Parier
  sword.add(part(capsule(0.022*s, 0.07*s, 10), hide(AZI.belt), 0, -0.025*s, 0));               // Griff
  sword.add(part(ball(0.032*s, 12), metal(AZI.gold, 0.3), 0, -0.075*s, 0));                    // Knauf
  ...
  root.userData.rig = { hips, torso, head, legL, legR, armL, armR, prop:sword,
                        poseL:-0.62, poseR:-0.12, height:1.2*s };
  return root;
}
